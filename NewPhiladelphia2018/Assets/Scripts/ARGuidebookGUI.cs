...
public class ARGuidebookGUI : MonoBehaviour
{
    ...
    public Texture ScreenshotButtonTexture;
    ...
    // Properties
    private VuforiaARController ARCamera
    {
        get { return VuforiaARController.Instance; }
    }
            ...
            { // only when tracking and no menus are on screen
              ...
              // Debug.Log(focusedPOI.Title);
                if (GUI.Button(new Rect(Screen.width - 20 - Screen.height / 10, 5, Screen.height / 10, Screen.height / 10), ScreenshotButtonTexture, "inventoryButton"))
                {
                    Debug.Log("Pressed");
                    if (focusedPOI != null)
                        ShareScreenshotWithText(string.Format("{0}, {1}", focusedPOI.Title, focusedPOI.Date));
                    else if (selectedTarget != null)
                        ShareScreenshotWithText(selectedTarget.Title);
                }
            }
#endif
        }
    ...
    private void DebugGUI()
    {
        ...
        if (SceneManager.sceneCountInBuildSettings > 1)
        {
            ...
            int level = GUILayout.Toolbar(SceneManager.GetActiveScene().buildIndex, names);
    ...
    private void CameraGUI()
    {
        ...
        if (ARCamera != null)
            GUILayout.Label("Viewport: " + ARCamera.GetVideoBackgroundRectInViewPort());
